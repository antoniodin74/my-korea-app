...
function App() {
  ...
  const HomePage = () => (
    ...
    >
      <div className="absolute top-8 right-8 z-50 pointer-events-none opacity-20 text-[10px] font-bold text-sky-900 tracking-[0.2em] uppercase">
        {isFullscreen ? 'Mode: Fullscreen' : 'Double click for Fullscreen'}
      </div>
      ...
      <div className="flex flex-wrap gap-16 justify-center items-center z-50">
        {cards && cards.map((card, index) => {
          // Gestione dinamica del click basata sull'indice
          let handleCardClick = null;
          if (index === 0) handleCardClick = () => navigate('/clienti');
          if (index === 1) handleCardClick = () => navigate('/clienti2');
          if (index === 2) handleCardClick = () => navigate('/analytics');
